        ...
        try:
            ...
            while True:
                if best_route is None:
                    best_route = self.ford_table[dest][0]
                else:
                    if best_route in self.cost_D.keys():
                        break
                    elif best_route == self.name:
                        best_route = dest
                        break
                    else:
                        best_route = self.ford_table[best_route][0]

            self.intf_L[self.cost_D[best_route][0]].put(p.to_byte_S(), 'out', True)
