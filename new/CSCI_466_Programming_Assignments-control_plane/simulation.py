...
if __name__ == '__main__':
    ...
    cost_D = {'H1': (0,1), 'H2':(2,1), 'RB': (1,1), 'RC': (3,5)} # {neighbor: {interface: cost}}
    router_a = network.Router(name='RA', 
                              cost_D = cost_D,
                              max_queue_size=router_queue_size)
    object_L.append(router_a)

    cost_D = {'RD': (1,3), 'RA': (0,5)} # {neighbor: {interface: cost}}
    ...
    cost_D = {'RB': (0, 5), 'RC': (2, 1), 'H3':(1,1)}  # {neighbor: {interface: cost}}
    ...
    link_layer.add_link(link.Link(host_2, 0, router_a, 2))
    link_layer.add_link(link.Link(host_3, 0, router_d, 1))
    link_layer.add_link(link.Link(router_a, 1, router_b, 0))
    link_layer.add_link(link.Link(router_a, 0, host_1, 0))
    link_layer.add_link(link.Link(router_a, 2, host_2, 0))
    ...
    link_layer.add_link(link.Link(router_c, 0, router_a, 3))
    link_layer.add_link(link.Link(router_d, 0, router_b, 1))
    link_layer.add_link(link.Link(router_d, 2, router_c, 1))
    link_layer.add_link(link.Link(router_d, 1, host_3, 0))
